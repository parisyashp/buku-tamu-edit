...
class Database{
	...
	public function tambah_tamu($nama, $handphone, $alamat, $instansi, $tujuan_kunjungan){
	    $tgl = date('Y-m-d');
	    ...
	    if($d['jumlah'] > 0){
	        ...
	        $query = $this->conn->query("INSERT INTO daftar_tamu(nama,hp,alamat,instansi,tujuan_kunjungan,tanggal) 
	                    VALUES('$nama', '$handphone', '$alamat', '$instansi', '$tujuan_kunjungan', now())");
	        if ($query = true){
	...
	public function daftar_tamu(){
	    $hasil = $this->conn->query("SELECT id,nama,hp,alamat,instansi,tujuan_kunjungan,tanggal from daftar_tamu");
